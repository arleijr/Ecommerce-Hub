#Import all the necessary libraries for the program to function.
...
#Sets the 'locale' library location to pt_BR. Used here to format monetary values in Brazilian Real.
locale.setlocale(locale.LC_ALL,'pt_BR.UTF-8')



#Class of functions for the main interface. Here, all the functions present in the 'Ecommerce Hub' interface are contained.
class UIFunction:
    def __init__(self):
        pass
    ...
    #Validates the information entered in the new user registration field.
    def cadastro(self):
        ok_name = False
        ok_user = False
        ok_password = False
        ...
        #Combobox to select the group in the registration area.
    ...
    #Sends the registration information to the file where the data is stored.
    ...
    #Function that performs user login.
    def login(self):
    ...
    #Returns the user group for validation.
    ...
    #Connects the clicked buttons to the functions of the main interface.
    def pressed_button(self, button):

        #Connects the clickable text 'sign up' to the function that goes to the signup page.
        if button == "label_4":
            UIFunction.page_cadastro(self)

        #Connects the button to the signup function.
        if button == "pushButton_10":
            UIFunction.cadastro(self)

        #Connects the login button to the login function.
        if button == "pushButton_9":
            UIFunction.login(self)

        #Opens the 'Actions' interface through the 'Actions' button.
        if button == "bn_acoes":
            self.w = Validador()
            self.w.show()
        ...
        #Opens the 'Dashboard' interface through the 'Dashboard' button.
        ...
        #Opens the reports interface.
        if button == "bn_relatorios":
            self.bases = Bases()
            self.bases.show()
        
        #Opens the 'Utilities' interface.
...
#Class for the Dashboard interface.
class Dashboard(QWidget):

    #Executes when the Dashboard is started.
    def __init__(self):
        super().__init__()
    ...
    #Executes when the Dashboard is opened for the first time.
    def showEvent(self, event):

        #Checks if it's the first time being opened.
        if self.first_time_open:
            
            
            #Hides some elements that won't be used initially.
            ...
            #Sets up today's date and the date available for user selection on the Dashboard.
            ...
            #Connects the Dashboard buttons to the 'buttonPressed' function, which assigns functionality to the buttons.
    ...
    #Executes the initial functions required for the program to run, such as table imports and calculations.       
...
#Class of functions for the Dashboard interface.
class Dashboard_func:
    def __init__(self):
        pass

    #Sets up the widget that initially appears.
    ...
    #Calculates and returns the sales values for the overall dashboard.
    ...
    #Checks which dates are selected and what type of value (sales, revenue) is selected on the dashboard.
    def verificar(self):
    ...
    #Selects from which marketplace the value should come.
    def set_marketplace(self, marketplace):
        self.mktp = marketplace
    

    #Performs calculations and returns individual dashboard data per marketplace.
    ...
    #Imports the necessary tables from the database to make the dashboard work.
    ...
    #Returns the selected date on the dashboard and formats it to retrieve the value from the table.
    def getdate(self):
        self.datefrom =  self.dashboard_ui.dateEdit.date()
        self.datetill =  self.dashboard_ui.dateEdit_2.date()
    ...
    #A function that connects the buttons to other functions within the program.
